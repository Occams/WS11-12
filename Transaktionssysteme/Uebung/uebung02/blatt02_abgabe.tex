...
\hypersetup{
	...
	pdftitle={Transaktionssysteme Übungsblätter},    % title
	pdfauthor={Huber Bastian},     % author
    pdfsubject={Loesung Transaktionssysteme},   % subject of the document
    pdfcreator={Daniel Watzinger},   % creator of the document
    pdfproducer={Daniel Watzinger}, % producer of the document
    pdfkeywords={Transaktionssysteme, Uni Passau, WS 2011/12}, % list of keywords
...
H_S(w_1 (y)) := f_1^y ( H_S(r_1 (y)) ) = f_1^y ( f_0^y() ) = f_0^y() + 1\\
H_S(r_3 (z)) := H_S( w_0(z) ) = f_0^z() \\
H_S(w_3 (z)) := f_3^z( H_S(r_3(z)) ) = f_3^z( f_0^z() ) = f_0^z() + 1\\
H_S(r_2 (x)) := H_S( w_0(x) ) = f_0^x() \\
H_S(w_2 (x)) := f_2^x( H_S(r_2 (x)), H_S(r_2 (y)) ) = f_2^x( f_0^x(),f_0^y() ) = f_0^x() * f_0^y()\\
H_S(w_1 (x)) := f_1^x( H_S(r_1 (y)) ) = f_1^x( f_0^y() ) = f_0^y() + 1 \\ \\
...
H_S(r_2 (y)) := H_S( w_0(y) ) = f_0^y() \\
H_S(r_2 (x)) := H_S( w_0(x) ) = f_0^x() \\
H_S(w_2 (y)) := f_2^y( H_S(r_2 (y)), H_S(r_2 (x)) ) = f_2^y( f_0^y(), f_0^x()  ) = f_0^y() *  f_0^x()\\
H_S(r_1 (x)) := H_S( w_0(x) ) = f_0^x() \\
H_S(w_1 (x)) := f_1^x( H_S(r_1 (x)) ) = f_1^x( f_0^x() ) = f_0^x() + 1 \\ \\
...
\subsection*{b)}
$
H_S[S](x) = H_S(w_0 (x)) = f_0^x() \\
H_S[S](y) = H_S(w_2 (y)) = f_2^y( f_0^x() ) \\
H_S[S](z) = H_S(w_2 (z)) = f_2^z( f_0^z(), f_0^x() ) \\
$
\subsection*{c)}
$
...
$
\subsection*{d)}
Es gilt zwar $H_S[S]$ = $H_S[S']$, aber op(S) $\neq$ op(S'). Somit sind S und S' nicht final state äquivalent.
...
H_S(r_1 (x)) := H_S( w_0(x) ) = f_0^x() \\
H_S(r_2 (y)) := H_S( w_0(y) ) = f_0^y() \\
H_S(w_2 (x)) := f_2^x( H_S(r_2 (y)) ) = f_2^x( f_0^y() )\\
H_S(r_3 (x)) := H_S( w_2(x) ) = f_2^x( f_0^y() ) \\
H_S(r_3 (z)) := H_S( w_0(z) ) = f_0^z() \\
H_S(w_3 (z)) := f_3^z( H_S(r_3 (z)) , H_S(r_3 (x)) ) = f_3^z( f_0^z() , f_2^x( f_0^y() ) ) \\
H_S(w_1 (z)) := f_1^z( H_S(r_1 (x)) ) = f_1^z( f_0^x() ) \\
$
\subsection*{b)}
...
\subsection*{c)}
$
S' = r_3(x) r_3(z) w_3(z) r_1(x) w_1(z) r_2(y) w_2(x)
$
...
$$
\subsection*{d)}
S $\in$ FSR , da mit S' ein serieller Schedule besteht, für den gilt: $$op(S) = op(S')$$ und $$LRF(S) = LRF(S')$$
