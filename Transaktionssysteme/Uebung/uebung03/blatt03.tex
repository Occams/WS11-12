...
Gegeben sei der Schedule $S := r_1(z)r_3(x)r_2(z)w_1(z)w_1(y)c_1w_2(y)w_2(z)c_2w_3(y)c_3$
	\begin{enumerate}[label={\alph*)}]
		\item
			\textit{Zeichnen Sie den Schrittgraphen $D(S)$ und kennzeichnen Sie die nicht lebendigen Schritte durch Unterstreichen.}
			
			
			\input{graph_1_a}
		\item
			\textit{Bestimmen Sie $RF(S)$ und $LRF(S)$.}
			...
			\item
				\textit{Stellen Sie die Konfliktrelation $conf(S)$ auf.}
			...
			$
			\item
				\textit{Zeichnen Sie den Konflikschrittgraphen $D_c (S)$.}
				
				
				\input{graph_1_d}
			\item
				\textit{Zeichnen Sie den Konfliktgraphen $G(S)$.}
				
				
				\input{graph_1_e}
			\item
				\textit{Prüfen Sie, ob $S$ in den Klassen $FSR$, $VSR$ bzw. $CSR$ enthalten ist.}
				...
				\begin{itemize}
					\item Teste $S_1' = T_1 T_2 T_3 = r_1(z) w_1(z) w_1(y) r_2(z) w_2(y) w_2(y) r_3(x) w_3(y)$. $(T_0, z, T_2) \in LRF(S) $ ist in $RF(S_1')$ nicht enthalten. Deshalb ist diese Reihenfolge nicht korrekt. Um $(T_0, z, T_2) \in RF(S_1')$ zu gewährleisten muss $T_2$ vor $T_1$ sein, denn $T_1$ schreibt auf $z$. Somit testen wir nur noch folgende drei Fälle:
...
Anhand von LRF(S) lassen sich für die seriellen Historien bereits einige Bedingungen heraus lesen:
	\begin{itemize}
...
Gegeben sei der Schedule $S := r_1(x)r_3(z)w_1(y)r_2(y)w_1(x)r_3(x)w_3(z)w_2(z)w_3(x)$
	\begin{enumerate}[label={\alph*)}]
		\item 
			\textit{Bestimmen Sie die Herbrand-Semantik des Datenobjekts $x$, d. h. $H[S](x)$.}
		...
		\item
			\textit{Zeichnen Sie den Schrittgraphen von $S$ und kennzeichnen Sie die nicht lebendigen Schritte durch Unterstreichen.}
			...
			\item
				\textit{Erstellen Sie den Konfliktgraphen von $S$.}
			...
			\item
				...
				\item
				\textit{Geben Sie eine Reihenfolge für die Commit-Operationen von $T1$ , $T2$ und $T3$ an, so dass $S \in COCSR$.}
...
Gegeben sei der Schedule $S := r_1(x)w_1(z)r_2(z)w_1(y)c_1r_3(y)w_2(z)c_2w_3(x)w_3(y)c_3$
	\begin{enumerate}[label={\alph*)}]
		...
		\item
			\textit{Zeigen Sie mithilfe des Konfliktgraphen $G(S)$, dass $S \in CSR$ gilt.}
				\input{graph_3_b}
				Der Konfliktgraph ist azyklisch, somit gilt $S \in CSR$
			
			
		\item 
			\textit{Prüfen Sie, ob ebenfalls $S \in OCSR$ bzw. $S \in COCSR$ gilt.}
