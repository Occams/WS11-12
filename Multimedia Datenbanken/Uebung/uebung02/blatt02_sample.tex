...
% Title
\def\thetitle{Multimedia Datenbanken --- Blatt 02 --- Musterlösung }
...
\section{Aufgabe 1}
	...
	\begin{itemize}
			...
			\item eigener, multimedialer Typ möglich
			\item MM - Objekt als eigenständiges Objekt in der Datenbank
		\end{itemize}
	\end{itemize}
...
Erweiterungen im ORDB - Modell:
	\begin{itemize}
		...
		\item{Anfragesprache}
	\begin{itemize}
			\item spezielle Operationen (Finden ähnlicher Bilder)
			\item spezielle Prädikate (überdeckt, liegt innerhalb)
		\end{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
						\item{Optimierung}
	\begin{itemize}
			\item Benutzer - definierte Kostenfunktionen für die neuen Datentypen
		\end{itemize}
	\end{itemize}
...
\begin{itemize}
	...
	\item Komplexität (benutzer - definierte Indexstrukturen, OO und relationale Kenntnisse erforderlich)
\end{itemize}
...
\section{Aufgabe 2}
	\begin{itemize}
		\item{Benutzer definierter Datentyp} \begin{itemize}
			...
			\item CREATE OR REPLACE TYPE adress\_type AS OBJECT (
	...
	city VARCHAR2(200)
		...
		\item CREATE OR REPLACE TYPE person\_type AS OBJECT (
	...
	\begin{itemize}
			...
			\item CREATE TYPE student\_type UNDER person\_type (
	...
	\begin{itemize}
			...
			\item CREATE TABLE student OF student\_type (
	...
	PRIMARY KEY (student\_id)
			...
			\item INSERT INTO students VALUES (
			
	David,
	
	Coquil,
	
	address\_type(Innstrasse,Passau),
	...
	\begin{itemize}
			...
			\item CREATE TYPE university\_type AS OBJECT (
...
CREATE TABLE university OF university\_type;
		\item INSERT INTO university VALUES (
	...
	student\_type(
		...
		address\_type(Ingram Street,New York City),
	...
	\begin{itemize}
			...
			\item CREATE TYPE person\_type2 AS OBJECT (
			
	first\_name VARCHAR2(50),
	
	last\_name VARCHAR2(50),
	
	address REF address\_type
	
);

CREATE TABLE address OF address\_type;

INSERT INTO address VALUES (Innstrasse, Passau);

CREATE TABLE person2 OF person\_type2;
...
INSERT INTO person2 VALUES (
	...
	(SELECT REF(a) FROM address a
	
	  	WHERE a.street LIKE Innstrasse AND   
	  	 	
		a.city LIKE Passau);
		
 );
			\item{1:n} Umsetzung mit nested tables (Tabelle, die als Attribut in einem Typ benutzt wird)
			\item CREATE TYPE nested\_address\_type AS TABLE OF address\_type;
			
			CREATE TYPE person\_nested AS OBJECT (
			
	first\_name VARCHAR2(50),
	
	last\_name VARCHAR2(50),
	
	address nested\_address\_type
	
);

CREATE TABLE person\_nested OF person\_type\_nested
        
	NESTED TABLE address STORE AS Address\_Table;
	
	INSERT INTO person\_nested VALUES (
	
	David, Coquil,
	
	nested\_address\_type(
		...
		\end{itemize}
					\item{DOT - Operator}
	\begin{itemize}
			\item Navigieren im Schema für Anfragen
			\item SELECT s.address.street, s.address.city FROM students s;
		\end{itemize}
					\item{REF}
	\begin{itemize}
	...
	\begin{itemize}
			...
			\item unabhängig vom Zustand des Objekts
		\end{itemize}
					\item{DEREF}
	\begin{itemize}
			\item Dereferenzieren
			\item SELECT p.address FROM person2 p WHERE p.last\_name LIKE Coquil;
		\end{itemize}
	\end{itemize}
\section{Aufgabe 3}
Siehe Beispiellösung in StudIP.
\section{Aufgabe 4}
Siehe Beispiellösung in StudIP.
\section{Aufgabe 5}
Siehe Beispiellösung in StudIP.
