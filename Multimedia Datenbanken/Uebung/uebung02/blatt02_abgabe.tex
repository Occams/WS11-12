...
\begin{description}
	...
	\item[objekt - relational] Alle Informationen werden in Tabellen mit komplexen Datentypen gespeichert. Diese Typen werden vom Benutzer erzeugt. Objekte sind Ausprägungen dieser Typen. Tabellen können solche Objekte speichern. Die Struktur liegt in den Typen.
\end{description}
\section{Aufgabe 2}
\begin{description}
	...
	\item[Polymorphismus] Überschreibung von Methoden in der Typvererbung.
	\item[Collections] Menge von Objekten desselben Typs.
	\item[Dot-Operator] Zugriff auf Attribute in Objekttabellen über diesen Operator (z.B. a.thema.id ,  wobei thema ein Typ ist und id ein Attribut von thema).
	\item[OID] Object Identifier, ein systemweit eindeutiger Schlüssel für Objekte.
	...
	\item[Beziehungen zwischen benutzer-definierten Datentypen (1:n, 1:1)] Beziehung zwischen Datentypen, die in einem ER oder UML Diagramm angegeben werden können (z.B. ein Auto hat mehrere Reifen, aber nur ein Farbe).
\end{description}
...
CREATE TYPE teilnehmer AS OBJECT (
  idno		NUMBER,
  vorname	VARCHAR(255),
  zuname	VARCHAR(255),
  geburtstag	DATE
...
CREATE_TYPE spieler_list AS TABLE OF REF spieler_typ;
CREATE TYPE spieler_typ UNDER teilnehmer (
  teams		team_list,  -- TODO: Unique (spieler, team) ausserdem hab ich 
			    -- das mit der teamzurodnung aus dem uml nicht verstanden
  aufstellungen	team_aufstellung_list
);

CREATE_TYPE team_list AS TABLE OF REF team_typ;
CREATE TYPE team_typ AS OBJECT (
  idno		NUMBER,
  name		VARCHAR(255),
  aufstellungen team_aufstellung_list
);

CREATE TYPE aufstellungsinfos_typ AS TABLE OF aufstellungsinfo_typ;
CREATE TYPE aufstellungsinfo_typ AS OBJECT (
  idno		NUMBER,
  position	VARCHAR2(1) CHECK( position IN ('T','V','M','S') ),
  spieler	aufstellungsinfos_typ
);

CREATE TYPE team_aufstellung_list AS TABLE OF REF team_aufstellung_typ;
CREATE TYPE team_aufstellung_typ AS OBJECT (
  idno		NUMBER,
  spieler	aufstellungsinfo_typ,
  team		REF team_typ,
  spiele	spiel_list
);

CREATE TYPE spiel_list AS TABLE OF REF spiel_typ;
CREATE TYPE spiel_typ AS OBJECT (
  idno		NUMBER,
  datum		DATE,
  heimtore	NUMBER,
  auswärtstore	NUMBER,
  home		REF team_aufstellung_typ,
  guest		REF team_aufstellung_typ
);
\end{lstlisting}
\subsection{b)}
\textit{
